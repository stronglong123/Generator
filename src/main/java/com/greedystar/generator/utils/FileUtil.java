...
public class FileUtil {
    ...
    public static void generateToJava(int type, Object data, String filePath) throws IOException, TemplateException {
        ...
        File fileDir = new File(file.getPath().substring(0, file.getPath().lastIndexOf("\\")));
        if(!fileDir.exists()){
            fileDir.mkdirs();
        }
    ...
    private static Template getTemplate(int type) throws IOException {
        switch (type) {
            ...
            case FreemarketConfigUtils.TYPE_BL:
                return FreemarketConfigUtils.getInstance().getTemplate("bl.ftl");
            case FreemarketConfigUtils.TYPE_TEST:
                return FreemarketConfigUtils.getInstance().getTemplate("Test.ftl");
            default:
                return null;
        }
    }
