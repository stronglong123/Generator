 ...
 */
public class BlTask extends AbstractTask {

    public BlTask(String className) {
        super(className);
    }
    ...
    public void run() throws IOException, TemplateException {
        ...
        String filePath = FileUtil.getSourcePath() + StringUtil.package2Path(ConfigUtil.getConfiguration()
                .getPackageName()) + StringUtil.package2Path(ConfigUtil.getConfiguration().getPath().getBl());
        String fileName;
        blData.put("Override", "");
        blData.put("InterfaceImport", "");
        fileName = className + "BL.java";
        System.out.println("Generating " + className + "BL.java");
        // 生成bl文件
        FileUtil.generateToJava(FreemarketConfigUtils.TYPE_BL, blData, filePath + fileName);
    }
}
